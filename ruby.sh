      ...
      5.10)
        ...
        if ((UID)); then
          if \which sudo >/dev/null 2>&1; then
            sudo_10=sudo
          elif \which /opt/csw/bin/sudo >/dev/null 2>&1; then
            sudo_10=/opt/csw/bin/sudo
          else
            fail "sudo is required but not found. You may install sudo from OpenCSW repository (https://www.opencsw.org/about)"
          fi
...
rvm_install_parse_params() {
  ...
  if ((${#install_gems[@]} > 0 && ${#install_rubies[@]} == 0)); then
    install_rubies=(ruby)
  fi
...
rvm_install_ruby_and_gems() (
  ...
  then
    ...
    for _ruby in ${install_rubies[@]}; do
      command rvm "${forwarded_flags[@]}" install ${_ruby}
    done
    ...
    for _gem in ${install_gems[@]}; do
      rvm "${forwarded_flags[@]}" all do gem install ${_gem}
    done
